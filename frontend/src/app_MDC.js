...
// API endpoints
const api = {
	createRoom: () => `${CONFIG.API_URL}/rooms`, // can also be used to check if roomId exists
	...
	return new Promise(resolve => {
		...
		answerPollingInterval = setInterval(async () => {
			try {
				...
				if (response.status === 200 && roomData?.answer) {
...
async function openUserMedia() {
	...
	try {
		...
		document.querySelector('#permissionBtn').style.display = 'none';
		document.querySelector('#joinBtn').disabled = false;
		document.querySelector('#createBtn').disabled = false;
		document.querySelector('#hangupBtn').disabled = true;
